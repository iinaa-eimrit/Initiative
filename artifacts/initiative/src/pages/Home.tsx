...
import { ArrowRight, Globe2, HeartHandshake, Target, Sparkles, Shield, BarChart3, Users, DollarSign, CheckCircle2, Lightbulb, Cpu, Handshake, Trophy } from "lucide-react";
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link href={user ? "/initiatives/new" : "/signup"}>
                  <Button size="lg" className="h-14 px-8 text-base rounded-full w-full sm:w-auto shadow-xl shadow-primary/25 hover:-translate-y-1 transition-transform">
                    Start Your Initiative <Sparkles className="ml-2 w-5 h-5" />
                  </Button>
                </Link>
                      ...
                      <Card className="rounded-3xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border-border/50 cursor-pointer bg-white h-full">
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <Badge variant="outline" className="text-xs rounded-full">{initiative.status}</Badge>
                          </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg">Real numbers from real initiatives making a difference.</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              { value: "4", label: "Categories Covered", icon: <Globe2 className="w-6 h-6" />, color: "text-amber-500" },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
                initial={{ opacity: 0, scale: 0.9 }}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href={user ? "/initiatives/new" : "/signup"}>
                <Button size="lg" className="h-14 px-10 text-base rounded-full shadow-xl shadow-primary/25">
                  {user ? "Create an Initiative" : "Get Started Free"}
                </Button>
              </Link>
              <Link href="/initiatives">
